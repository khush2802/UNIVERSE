...
export default async function ProjectPage({ params }: PageProps) {
  ...
  const sections = [
    ...
    { id: 'data-layer', label: 'Data layer', show: project.dataLayer.kind !== 'unknown' },
  ...
  return (
    <article className="mx-auto max-w-4xl px-6 py-32">
      ...
      {project.dataLayer.kind !== 'unknown' && (
        <section id="data-layer" className="mt-16 scroll-mt-40">
          <h2 className="text-[length:var(--text-xl)] font-semibold">Data layer</h2>
          <p className="label-technical mt-4">Visualisation arrives in chunk 09</p>
          <p className="mt-3 text-[var(--color-ink-muted)]">
            {project.dataLayer.kind === 'none'
              ? 'No persistent database detected. This project does not appear to require one.'
              : `Detected: ${describeDataLayer(project)}`}
          </p>
        </section>
      )}
...
function describeDataLayer(project: Project): string {
  const layer = project.dataLayer;

  switch (layer.kind) {
    case 'sql':
      return `${layer.engine}, ${layer.tables.length} tables`;
    case 'document':
      return `${layer.engine}, ${layer.collections.length} collections`;
    case 'vector':
      return `${layer.store}, ${layer.pipeline.length}-stage retrieval`;
    case 'keyvalue':
      return `${layer.engine} (${layer.roles.join(', ')})`;
    default:
      return 'unknown';
  }
}
